                            ...
                            <div class="group relative">
                                <div
                                    ...
                                    <img style="width: 100px;"
                                         src="{{ asset($order->product->image) }}" alt="Front of men&#039;s Basic Tee in black."
                                         class="h-2 w-2 object-cover object-center lg:h-full lg:w-full">
                                </div>
                                <div class="mt-4 flex justify-between">
                                    <div>
                                        <h3 class="text-sm text-gray-700">
                                            <a href="{{ route('buy-now',$order->product ) }}">
                                                <span aria-hidden="true" class="absolute inset-0"></span>
                                                {{ $order->product->name }}
                                            </a>
                                        </h3>
                                        <p class="mt-1 text-sm text-gray-500">{{ $order->product->description }}</p>
                                    </div>
                                    <p class="text-sm font-medium text-gray-900">{{ $order->product->price }}</p>
                                </div>
                                <div>
